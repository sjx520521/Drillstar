...
class LSTMWorker(QThread):
    ...
    def run(self):
        if not _TORCH_AVAILABLE:
            self.failed.emit("LSTM 需要 PyTorch，请先安装：pip install torch")
            return
        try:
            import torch
            import torch.nn as nn
            ...
            result_df.loc[pred_indices, "LSTM_Prediction"] = y_all_pred
            ...
            summary_lines = [
                ...
                f"序列长度：{seq_len}",
                f"隐藏单元：{self.params['hidden_size']}  层数：{self.params['num_layers']}",
                f"训练轮次：{epochs}",
                f"测试集 RMSE：{rmse:.4f}",
                f"测试集 MAE：{mae:.4f}",
                f"测试集 R²：{r2:.4f}",
...
class Win_ModelLSTM(QWidget):
    ...
    def _build_ui(self):
        self.setWindowTitle("DataAnalysis [LSTM]")
        ...
        self.spin_seq_len.setValue(10)
        param_layout.addRow("序列长度", self.spin_seq_len)

        self.spin_hidden_size = QSpinBox()
        self.spin_hidden_size.setRange(8, 1024)
        self.spin_hidden_size.setValue(64)
        param_layout.addRow("隐藏单元数", self.spin_hidden_size)

        self.spin_num_layers = QSpinBox()
        self.spin_num_layers.setRange(1, 8)
        self.spin_num_layers.setValue(2)
        param_layout.addRow("LSTM 层数", self.spin_num_layers)
        ...
        if not _TORCH_AVAILABLE:
            self._update_result_text("警告：未检测到 PyTorch，LSTM 功能不可用。\n请安装：pip install torch")
        else:
    ...
    def _set_busy(self, busy: bool):
        ...
        self.spin_hidden_size.setEnabled(not busy)
        ...
        self.label_busy.setText("正在训练 LSTM 模型，请稍候。" if busy else "当前空闲。")
    ...
    def on_Load(self):
        ...
        try:
            ...
            if not isinstance(value, pd.DataFrame) or value.empty:
                QMessageBox.warning(self, "警告", "加载的数据为空，无法进行 LSTM 分析。")
                return
            self.df = value.copy()
            ...
            self.df_result = pd.DataFrame()
            self.plot_widget.clear("Run LSTM to show the prediction chart.")
            self.btn_translate.setEnabled(False)
            ...
            self.node.eval()
            QMessageBox.information(self, "成功", "LSTM 数据加载完成。")
        except Exception as exc:
            self.node.markInvalid()
            QMessageBox.critical(self, "错误", f"LSTM 数据加载失败：{exc}")

    def on_Create(self):
        if not _TORCH_AVAILABLE:
            QMessageBox.critical(self, "错误", "未安装 PyTorch，无法运行 LSTM。\n请执行：pip install torch")
            return
        if self._busy:
    ...
    def _on_worker_failed(self, message: str):
        self.df_result = pd.DataFrame()
        self.plot_widget.clear("LSTM failed.")
        self._update_result_text("LSTM 计算失败。\n\n" + message)
        QMessageBox.critical(self, "错误", f"LSTM 计算失败：{message}")
    ...
    def on_Transfer(self):
        ...
        try:
            ...
            QMessageBox.information(self, "提示", "LSTM 结果已传递。")
        except Exception as exc:
            QMessageBox.critical(self, "错误", f"LSTM 结果传递失败：{exc}")
