...
class LogisticRegressionWorker(QThread):
    ...
    def run(self):
        try:
            ...
            result_df["LR_Prediction"] = ""
            result_df.loc[valid_mask, "LR_Prediction"] = predicted_labels

            proba = model.predict_proba(x_valid).max(axis=1)
            result_df["LR_Confidence"] = pd.NA
            result_df.loc[valid_mask, "LR_Confidence"] = proba

            result_df["LR_IsCorrect"] = pd.NA
            result_df.loc[valid_mask, "LR_IsCorrect"] = predicted_labels == y_valid.to_numpy()
            ...
            summary_lines = [
                ...
                f"正则化：{self.params['penalty']}  C={self.params['c_value']}",
                f"求解器：{self.params['solver']}",
...
class Win_ModelLogisticRegression(QWidget):
    ...
    def _build_ui(self):
        self.setWindowTitle("DataAnalysis [Logistic Regression]")
        ...
        self.combo_penalty.addItems(["l2", "l1", "elasticnet", "none"])
        param_layout.addRow("正则化", self.combo_penalty)

        self.combo_solver = QComboBox()
        self.combo_solver.addItems(["lbfgs", "liblinear", "saga", "newton-cg", "sag"])
        param_layout.addRow("求解器", self.combo_solver)

        self.spin_c = QDoubleSpinBox()
        self.spin_c.setRange(0.001, 100000.0)
    ...
    def _set_busy(self, busy: bool):
        ...
        self.combo_penalty.setEnabled(not busy)
        ...
        self.label_busy.setText("正在执行逻辑回归训练，请稍候。" if busy else "当前空闲。")
    ...
    def on_Load(self):
        ...
        try:
            ...
            if not isinstance(value, pd.DataFrame) or value.empty:
                QMessageBox.warning(self, "警告", "加载的数据为空，无法进行逻辑回归分析。")
                return
            self.df = value.copy()
            ...
            self.node.eval()
            QMessageBox.information(self, "成功", "逻辑回归数据加载完成。")
        except Exception as exc:
            self.node.markInvalid()
            QMessageBox.critical(self, "错误", f"逻辑回归数据加载失败：{exc}")
    ...
    def on_Create(self):
        ...
        params = {
            ...
            "penalty": self.combo_penalty.currentText(),
        ...
        self.worker = LogisticRegressionWorker(self.df, target_column, feature_columns, params)
        self.worker.finished_ok.connect(self._on_worker_finished)
        self.worker.failed.connect(self._on_worker_failed)
        self.worker.finished.connect(self._on_worker_cleanup)
        self._set_busy(True)
        self._update_result_text("逻辑回归正在后台计算，请稍候...")
        self.worker.start()
    ...
    def _on_worker_finished(self, payload):
        ...
        self.model_artifact = build_model_artifact(
            ...
            output_prefix="LogisticRegression",
        )
        self._update_result_text(payload["summary_text"])

    def _on_worker_failed(self, message: str):
        self.df_result = pd.DataFrame()
        self._update_result_text("逻辑回归计算失败。\n\n" + message)
        QMessageBox.critical(self, "错误", f"逻辑回归计算失败：{message}")
    ...
    def on_Transfer(self):
        ...
        try:
            ...
            QMessageBox.information(self, "提示", "逻辑回归结果已传递。")
        except Exception as exc:
            QMessageBox.critical(self, "错误", f"逻辑回归结果传递失败：{exc}")
