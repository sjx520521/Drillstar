...
class Scene(Serializable):
    ...
    def removeNode(self, node: Node):
        """增强：删除节点时，先删除其所有关联连线（适配Socket的edges属性），再移除节点"""
        try:
            # 第一步：删除该节点的所有关联连线
            for socket in node.inputs + node.outputs:
                # 只处理edges属性，彻底抛弃edge属性的引用
                if hasattr(socket, 'edges') and socket.edges:
                    # 遍历副本避免遍历中修改列表
            ...
            if node in self.nodes:
                ...
                if DEBUG_REMOVE_WARNINGS:
                    print("!W:", "Scene::removeNode", "wanna remove node", node,
                          "from self.nodes but it's not in the list!")

            # 第三步：移除节点的图形项
            if node.grNode and node.grNode.scene():
                node.grNode.scene().removeItem(node.grNode)
    ...
    def removeEdge(self, edge: Edge):
        """增强：删除连线时，先断开与两端Socket的关联（适配Socket的edges属性），再移除连线"""
        try:
            # 第一步：断开连线与两端Socket的关联
            if edge.start_socket:
                if hasattr(edge.start_socket, 'edges') and edge in edge.start_socket.edges:
                    edge.start_socket.edges.remove(edge)
                # 兼容：如果有edge属性也清空（避免旧代码残留）
                if hasattr(edge.start_socket, 'edge'):
                    edge.start_socket.edge = None

            if edge.end_socket:
                if hasattr(edge.end_socket, 'edges') and edge in edge.end_socket.edges:
                    edge.end_socket.edges.remove(edge)
                # 兼容：如果有edge属性也清空
                if hasattr(edge.end_socket, 'edge'):
            ...
            # 第三步：移除连线的图形项
            if edge.grEdge and edge.grEdge.scene():
                edge.grEdge.scene().removeItem(edge.grEdge)
    ...
    # 可选：辅助方法（若需要）
    def removeEdgeFromSocket(self, socket):
        """清理指定Socket的关联连线（适配edges属性）"""
        if hasattr(socket, 'edges') and socket.edges:
            for edge in socket.edges[:]:
                self.removeEdge(edge)
        # 兼容旧代码
        if hasattr(socket, 'edge') and socket.edge:
            self.removeEdge(socket.edge)
    ...
    def setNodeClassSelector(self, class_selecting_function: 'function') -> 'Node class type':
        """
